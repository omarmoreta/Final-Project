...
export default class MainmenuScene extends Phaser.Scene {
  ...
  preload() {
    this.load.html("nameform", "./users/loginform.html");
  }
  create() {
    // MUSIC
    ...
    WebFont.load({
      ...
      active: function () {
        const maintitle = add.text(
          screenCenterX / 2 - 30,
          screenCenterY + 50,
          game.config.gameTitle,
          { fontFamily: "Rubik Glitch", fontSize: 24, fill: "#DD636E" }
        );
        const gameversion = add.text(5, 5, "", {
          fontFamily: "Finger Paint",
          fontSize: 12,
          fill: "#ffffff",
        });
        gameversion.setText([game.config.gameVersion]);
      },
    });
    // START INSTRUCTIONS
